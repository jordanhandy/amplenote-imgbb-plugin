    ...
    "Upload all note images to ImgBB":{
      run: async function(app,noteUUID){
        ...
        try{
          ...
          const matches = noteContent.match(/!\[[^\]]*\]\((.*?)\)/gm);
          if(matches && matches.length > 0){
            for(let match of matches){
              console.log('this is the match',match.url)
              noteContent = noteContent.replace(match.url, await this._uploadFile(app, match.url));
            }
          }
